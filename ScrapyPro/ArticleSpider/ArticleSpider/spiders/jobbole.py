...
class JobboleSpider(scrapy.Spider):
    ...
    # def __init__(self):
    #     self.browser = webdriver.Chrome(executable_path='E:/PythonEnvs/selenium_chrome_driver/chromedriver.exe')
    ...
    def parse(self, response):
        ...
        # while self.next_count <= 10:
            # 提取下一页并交给scrapy进行下载
        next_urls = response.css(".next.page-numbers::attr(href)").extract_first("")
        if next_urls:
            yield Request(url=parse.urljoin(response.url, next_urls), callback=self.parse)

        # print('END %s page crawl' % self.next_count)
    ...
    def parse_detail(self, response):
        ...
        # else:
        #     fav_nums = 0
        # comments_nums = response.css("a[href='#article-comment']::text").extract()[0]
        # match_re = re.match(r'.*?(\d+).*', comments_nums)
        # if match_re:
